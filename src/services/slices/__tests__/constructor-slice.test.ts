...
import constructortReducer, { addIngredient, deleteIngredient, IConstructorState, moveIngredient } from "../constructor-slice" 
...
describe('change position of ingredient', () => {
  ...
  const state: IConstructorState = {
    constructorItems: {
      bun: null,
      ingredients: mockIngredients.map((x, ind) => ({ ...x, _id: ind.toString(), id: ind.toString() }) as TConstructorIngredient)
    }
  };
  const moving = state.constructorItems.ingredients[0];
  const movedState = constructortReducer(state, moveIngredient({ ingredient: moving, direction: 'down' }));
  ...
  test('move up', () =>{
    ...
    const movedState = constructortReducer(state, moveIngredient({ingredient: moving, direction: 'up'}));
  ...
  test('move down when boundary', () =>{
    ...
    expect(movedState.constructorItems.ingredients[length - 2]).toEqual(state.constructorItems.ingredients[length - 2]);
  })
  ...
  test('move up when boundary', () =>{
    ...
    expect(movedState.constructorItems.ingredients[1]).toEqual(state.constructorItems.ingredients[1]);
  })
